...
for(sp in 1:length(Spec.list)) {
  ...
  if(type == "m1") g1 <- ppc_hist(y, yrep[runif(8, 1, nsim),]) + ggtitle(dat.spec$species_eng[dat.spec$species == Spec.list[sp]])
  if(type != "m1") g1 <- ppc_hist(y, yrep[runif(8, 1, nsim),]) + ggtitle(Spec.list[sp])
  ...
  ggsave(paste0("../graphs/prefig/PPC_", type, "_", Spec.list[sp], "_", RAD, "m.jpeg"), 
         plot = grid.arrange(g1, arrangeGrob(gmean, gsd, gpropZ, gmax, ncol = 2, nrow = 2), nrow = 1), 
         width = 9, height = 4, newpage = F)
