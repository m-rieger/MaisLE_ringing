...
parameters { // log-transformed scale
  matrix[Nsp,Nyear] alpha; // year and species-specific intercepts
  matrix[Nsp,Nland] beta; // species-specific slope for dist, propM, propW, height, weed
  matrix[Nsp,2] gamma; // species-specific slopes for julian day

  vector<lower=0, upper=1>[Nsp] theta; // needed for species-specific zi-intercept

  vector<lower=0.00001> [Nsp] phi; // species-specific parameter for overdispersion (nbin)
...
transformed parameters{
  ...
  // for(s in 1:Nsp) k[s,] = L_SIGMA * z[s,];
  ...
  // vector[Nsp] theta ;
  // for(i in 1:Nsp) theta[i] =  inv_logit(alpha_theta + sp_theta[i]) ; //<lower=0, upper=1>
  
  vector[N] lambda; // expected number of inds per day and net
  for(n in 1:N) {
    lambda[n] = alpha[sp[n], year[n]] + // species-specific intercept per year
    beta[sp[n],1]*land[n,1] + // species-specific slopes of distance
    beta[sp[n],2]*land[n,2] + // species-specific slopes of propM
    beta[sp[n],3]*land[n,3] + // species-specific slopes of propW
    beta[sp[n],4]*land[n,4] + // species-specific slopes of height
    //beta[sp[n],5]*land[n,5] + // species-specific slopes of weed // land = 5
    gamma[sp[n],1]*jday[n] + // linear effect of julian day
    gamma[sp[n],2]*jday[n]*jday[n] + // quadratic effect of julian day
    k[site[n]] + // random site-specific intercept per species
    log(effort[n]); // offset of effort
  }
}
...
model {
  ...
  phi ~ exponential(1); // prior on phi (overdispersion)
  
  // alpha_theta ~ normal(0,1);
  // sp_theta    ~ normal(0,1);
  // ggf beta prior für theta

  // LIKELIHOOD neg_binomial_2_log_lpmf(ints n | reals eta, reals phi)
  for (n in 1:N) {

    if (y[n] == 0) {
      target += log_sum_exp(log(theta[sp[n]]), log1m(theta[sp[n]]) + neg_binomial_2_log_lpmf(y[n] | lambda[n], phi[sp[n]]));
    } else {
      target += log1m(theta[sp[n]]) + neg_binomial_2_log_lpmf(y[n] | lambda[n], phi[sp[n]]);
    }
  }
}
...
generated quantities{
  ...
  for(n in 1:N){
	     zero = bernoulli_rng(theta[sp[n]]);
	     y_sim[n] = (1-zero)*neg_binomial_2_log_rng(lambda[n], phi[sp[n]]);
  }
}
