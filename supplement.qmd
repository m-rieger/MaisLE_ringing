...
## add area_ha
site.sum <- left_join(site.sum, dat.site[, c("site_short", "area_ha")], by = "site_short")

site.sum <- site.sum[order(site.sum$site_short),]

site.sum$area_ha <- round(site.sum$area_ha, 1)

site.sum <- site.sum[, c("site_short", "lon", "lat", "area_ha", "nets", "e1000", "dist", "weed", "m500", "w500")]
colnames(site.sum) <- c("site", "lon", "lat", "area [ha]", "nets", "effort", "dist", "weed", "maize", "AST")

kbl(site.sum, caption = "Overview of study <em>sites</em>, including location (<em>lon</em>, <em>lat</em>), <em>area</em>, number of used <em>nets</em> and total <em>effort</em> in 1000 hm², as well as distance to field margin (<em>dist</em>), <em>weed</em> infestation and <em>maize</em> and <em>AST</em> (area covered with shrubs and trees) cover within a 500 m radius (mean (min-max)).",     
