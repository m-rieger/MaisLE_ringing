---
title: "Supplementary material of 'Maize fields as post-breeding habitat for songbirds: influence of landscape composition, field margins, and weed infestation'"
date: last-modified
author:
  - name: Mirjam R. Rieger
    email: mrieger@posteo.de
    orcid: 0000-0002-7404-6023
    corresponding: true
    affiliations: 
      - id: HFR
        name: Hochschule für Forstwirtschaft Rottenburg
        city: Schadenweilerhof, 72108 Rottenburg am Neckar
        country: Germany
  - name: Wieland Heim
    email: wieland.heim@uol.de
    orcid: 0000-0002-3262-2491
    affiliations:
      - id: UOL
        name: Carl von Ossietzky Universität Oldenburg, Institut für Biologie und Umweltwissenschaften
        city: Ammerländer Heerstraße 114-118, 26129 Oldenburg
        country: Germany  
  - name: László Kövér
    email: koverl@agr.unideb.hu
    orcid: 0000-0001-5368-3691
    affiliations:
      - id: UD
        name: University of Debrecen, Department of Nature Conservation Zoology and Game Management
        city: Boszormenyi str. 138, 4032 Debrecen
        country: Hungary  
  - name: Thomas K. Gottschalk
    email: gottschalk@hs-rottenburg.de
    orcid: 0000-0002-8951-386X
    affiliations:
      - ref: HFR
execute:
  echo: false
format: 
  html:
      embed-resources: true
toc: true
toc-depth: 2
toc-location: left
number-sections: true
number-depth: 3

---

<style>
.scroll5 {
    height: 500px;
    overflow-y: scroll;
}
</style>

<style>
.scroll4 {
    height: 400px;
    overflow-y: scroll;
}
</style>

```{r setup, include = FALSE}
library(kableExtra)
library(tidyverse)
library(plotly)
library(glue)

source("./R/utils.R")

dens   <- "density [ind. per 1000 hm²]"
dens2  <- "density"
nI     <- "no. of individuals"
nS     <- "no. of species"
dist   <- "distance to field margin [m]"
maize500  <- "prop. of maize within 0.5 km [prop.]"
woodS500  <- "prop. of AST within 0.5 km [prop.]"
maize750  <- "prop. of maize within 0.75 km [prop.]"
woodS750  <- "prop. of AST within 0.75 km [prop.]"
maize1000  <- "prop. of maize within 1 km [prop.]"
woodS1000  <- "prop. of AST within 1 km [prop.]"
weed   <- "weed infestation [prop.]"  
spec   <- "species"
year   <- "year"
jday   <- "decade of the year [10-day intervall]"
site   <- "study site"


```

```{=html}
<style>body {text-align: justify}</style>
```
<!-- this aligns the text to justified text (Blocksatz) -->

# Material and Methods

## Study sites
```{r, results='asis', echo = F}
dat.mod <- read.csv("./output/data.csv", encoding = "latin1")
dat.site <- read.csv("./data/dat_site.csv", encoding = "latin1")
dat.mod <- dat.mod[dat.mod$species == "Blaumeise",]
dat.mod$weed_p <- dat.mod$prop_weed/100
dat.mod$maize_500m <- dat.mod$maize_500m/100
dat.mod$maize_750m <- dat.mod$maize_750m/100
dat.mod$maize_1000m <- dat.mod$maize_1000m/100
dat.mod$woodS_500m <- dat.mod$woodS_500m/100
dat.mod$woodS_750m <- dat.mod$woodS_750m/100
dat.mod$woodS_1000m <- dat.mod$woodS_1000m/100

site.sum <- dat.mod %>% group_by(site_short) %>% 
  summarize(lon = round(mean(lon), 4),
            lat = round(mean(lat), 4),
            nets = length(unique(netNr)),
            e1000 = round(sum(effort)/1000, 1),
            dist = paste0(round(mean(distance_m), 0), " (", round(min(distance_m), 0), "-", round(max(distance_m), 0), ")"),
            weed = paste0(round(mean(weed_p), 2), " (", round(min(weed_p), 2), "-", round(max(weed_p), 2), ")"),
            m500 = paste0(round(mean(maize_500m), 2), " (", round(min(maize_500m), 2), "-", round(max(maize_500m), 2), ")"),
            #m750 = paste0(round(mean(maize_750m), 2), " (", round(min(maize_750m), 2), "-", round(max(maize_750m), 2), ")"),
            #m1000 = paste0(round(mean(maize_1000m), 2), " (", round(min(maize_1000m), 2), "-", round(max(maize_1000m), 2), ")"),
            w500 = paste0(round(mean(woodS_500m), 2), " (", round(min(woodS_500m), 2), "-", round(max(woodS_500m), 2), ")"),
            #w750 = paste0(round(mean(woodS_750m), 2), " (", round(min(woodS_750m), 2), "-", round(max(woodS_750m), 2), ")"),
            #w1000 = paste0(round(mean(woodS_1000m), 2), " (", round(min(woodS_1000m), 2), "-", round(max(woodS_1000m), 2), ")")
            )

## add area_ha
site.sum <- left_join(site.sum, dat.site[, c("site_short", "area_ha", "scheme")], by = "site_short")

site.sum <- site.sum[order(site.sum$site_short),]

site.sum$area_ha <- round(site.sum$area_ha, 1)

site.sum <- site.sum[, c("site_short", "scheme", "lon", "lat", "area_ha", "nets", "e1000", "dist", "weed", "m500", "w500")]
colnames(site.sum) <- c("site", "scheme", "lon", "lat", "area [ha]", "nets", "effort", "dist", "weed", "maize", "AST")
site.sum$scheme[site.sum$scheme == "professional"] <- "P"
site.sum$scheme[site.sum$scheme == "voluntary"] <- "V"

kbl(site.sum, caption = "Overview of study <em>sites</em>, including <em>scheme</em> ('P' = professional, 'V' = volunteer-based), location (<em>lon</em>, <em>lat</em>), <em>area</em>, number of used <em>nets</em> and total <em>effort</em> in 1000 hm², as well as distance to field margin (<em>dist</em>), <em>weed</em> infestation and <em>maize</em> and <em>AST</em> (area covered with shrubs and trees) cover within a 500 m radius (mean (min-max)).",     
    row.names = F, escape = FALSE) %>%
  kable_styling(full_width = FALSE, position = "left") %>%
  scroll_box(width = "900px", height = "750px") %>%

  row_spec(0:nrow(site.sum), extra_css = "font-size:14px;") %>%
  
  row_spec(which(1:nrow(site.sum)%%2!=1),
           background = "#CCCCCC") %>%
  row_spec(0, extra_css = "border-bottom: 1px solid black;", background = "#CCCCCC") %>%
  row_spec(nrow(site.sum), extra_css = "border-bottom: 1px solid black;")

```


## Effort
![Overview of annual trapping effort per decade of the year. Numbers in brackets give the total number of sites per year. *For reference: end of July/start of August = decade 21, end of August/start of September = decade 24, end of September/start of October = decade 27, end of October/start of November = decade 30.*](graphs/Effort.png){#fig-eff width="70%"}   

## OSM attributes

```{r, results='asis', echo = F}
df.osm <- read.csv("./data/OSM_attributes.csv")

# Button must be created inside the same chunk or just before the table output
# cat('<input type="button" class="hideshow">') # does only halfway work

kbl(df.osm, caption = "OpenStreetMap attributes used for landscape shares (<em>key</em>, <em>value</em>), type of geometry used (<em>points, polygons, multipolygons, lines, multilines</em>), assigned <em>landscape</em>, and <em>buffer</em> in m used to create polygons from point, line and mutliline objects.",     
    row.names = F, escape = FALSE) %>%
  kable_styling(full_width = FALSE, position = "left") %>%
  scroll_box(width = "900px", height = "750px") %>%

  row_spec(0:nrow(df.osm), extra_css = "font-size:14px;") %>%

  row_spec(which(1:nrow(df.osm)%%2!=1),
           background = "#CCCCCC") %>%
  row_spec(0, extra_css = "border-bottom: 1px solid black;", background = "#CCCCCC") %>%
  row_spec(nrow(df.osm), extra_css = "border-bottom: 1px solid black;")

```

## Predictors

```{r}


# plot predictor per predictor
plot2 <- function(x = NULL, y = NULL, color = "year", c.label = year,
                  data = dat.mod, 
                  xlab = NULL, ylab = NULL, basesize = 12){
  
  data$x     <- data[,x]
  data$y     <- data[,y]
  data$color <- as.factor(data[,color])
  
  g <- ggplot(data) +
    # plot points (jittered)
    geom_point(aes(x = x, y = y, color = color), pch = 1, size = 2) +
    # define color range of color-variable
    scale_color_viridis_d(c.label, direction = 1, end = 0.9, option = "inferno", alpha = 0.5) +
    # axis labels
    xlab(xlab) +
    ylab(ylab) +
    theme_light(base_size = basesize*1.5)
  
  if(x == "julian_day"){
    x.val <- c(21.3, 24.4, 27.4, 30.5)
    x.lab <- c("1st Aug", "1st Sep", "1st Oct", "1st Nov")

    g <- g + scale_x_continuous(sec.axis = dup_axis(
      name = "date",
      labels = x.lab,
      breaks = x.val
    ))

  }
  
  return(g)
}
```

::: {.panel-tabset}


### decade & distance
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(y = "distance_m", ylab = dist, x = "julian_day", xlab = jday)
```

### decade & maize

:::: {.panel-tabset}

#### 0.5 km
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(y = "maize_500m", ylab = maize500, x = "julian_day", xlab = jday)
```

#### 0.75 km
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(y = "maize_750m", ylab = maize750, x = "julian_day", xlab = jday)
```

#### 1 km
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(y = "maize_1000m", ylab = maize1000, x = "julian_day", xlab = jday)
```

::::

### decade & AST

:::: {.panel-tabset}

#### 0.5 km
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(y = "woodS_500m", ylab = woodS500, x = "julian_day", xlab = jday)
```

#### 0.75 km
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(y = "woodS_750m", ylab = woodS750, x = "julian_day", xlab = jday)
```

#### 1 km
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(y = "woodS_1000m", ylab = woodS1000, x = "julian_day", xlab = jday)
```

::::

### decade & weed
```{r Vwej, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(y = "weed_p", ylab = weed, x = "julian_day", xlab = jday)
```


### distance & maize

:::: {.panel-tabset}

#### 0.5 km
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(x = "distance_m", xlab = dist, y = "maize_500m", ylab = maize500)
```

#### 0.75 km
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(x = "distance_m", xlab = dist, y = "maize_750m", ylab = maize750)
```

#### 1 km
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(x = "distance_m", xlab = dist, y = "maize_1000m", ylab = maize1000)
```

::::

### distance & AST

:::: {.panel-tabset}

#### 0.5 km
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(x = "distance_m", xlab = dist, y = "woodS_500m", ylab = woodS500)
```

#### 0.75 km
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(x = "distance_m", xlab = dist, y = "woodS_750m", ylab = woodS750)
```

#### 1 km
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(x = "distance_m", xlab = dist, y = "woodS_1000m", ylab = woodS1000)
```

::::
  
### distance & weed
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(x = "distance_m", xlab = dist, y = "weed_p", ylab = weed)
```


### maize & AST

:::: {.panel-tabset}

#### 0.5 km
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(x = "maize_500m", xlab = maize500, y = "woodS_500m", ylab = woodS500)
```

#### 0.75 km
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(x = "maize_750m", xlab = maize750, y = "woodS_750m", ylab = woodS750)
```

#### 1 km
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(x = "maize_1000m", xlab = maize1000, y = "woodS_1000m", ylab = woodS1000)
```

::::

### maize & weed

:::: {.panel-tabset}

#### 0.5 km
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(x = "maize_500m", xlab = maize500, y = "weed_p", ylab = weed)
```

#### 0.75 km
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(x = "maize_750m", xlab = maize750, y = "weed_p", ylab = weed)
```

#### 1 km
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(x = "maize_1000m", xlab = maize1000, y = "weed_p", ylab = weed)
```

::::


### AST & weed

:::: {.panel-tabset}

#### 0.5 km
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(x = "woodS_500m", xlab = woodS500, y = "weed_p", ylab = weed)
```

#### 0.75 km
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(x = "woodS_750m", xlab = woodS750, y = "weed_p", ylab = weed)
```

#### 1 km
```{r, echo = F, warning = F, message = F, fig.dim = c(10, 5)}
plot2(x = "woodS_1000m", xlab = woodS1000, y = "weed_p", ylab = weed)
```

::::

:::

## Trait groups
```{r, results='asis', echo = F}
dat.spec <- read.csv("./output/data_Traits.csv", encoding = "latin1")
dat.spec <- dat.spec[order(dat.spec$species_sci), ]

dat.mod <- read.csv("./output/data.csv", encoding = "latin1")
dat.mod <- dat.mod %>% group_by(species) %>%
  summarize(nInd = sum(NInd_pd), .groups = "drop")
dat.mod$Ind_prop <- round(dat.mod$nInd/sum(dat.mod$nInd)*100, 1)

dat.mod$nInd_prop <- paste0(dat.mod$nInd, " (", dat.mod$Ind_prop, "%)")

## merge with dat.spec
dat.spec <- left_join(dat.spec, dat.mod[, c("species", "nInd_prop")], by = "species")
dat.spec <- dat.spec[, c("species_sci", "species_eng", "nInd_prop", "Migration", "Habitat", "Habitat.Density", "Primary.Lifestyle", "Trophic.Niche", "RedList_DE")]

colnames(dat.spec) <- c("scientific", "english", "individuals", "migration", "habitat", "habitat density", "primary lifestyle", "Trophic Niche", "Red List GER")

kbl(dat.spec, caption = "Overview of species trapped in maize fields, the number of trapped birds (including recaptures, share of total captures across all species given in brackets) and their attribution to different trait groups. <em>Habitat density levels: 1 = dense, 2 = semi-open, 3 = open. Red List categories: * = least concern, 2 = endangered, 3 = vulnerable, 4 = near threatened</em>",     
    row.names = F, escape = FALSE) %>%
  kable_styling(full_width = FALSE, position = "left") %>%
  scroll_box(width = "900px", height = "750px") %>%

  row_spec(0:nrow(dat.spec), extra_css = "font-size:14px;") %>%

  add_header_above(c("species name" = 2, " " = 1, "trait" = 6)) %>%
  row_spec(which(1:nrow(dat.spec)%%2!=1),
           background = "#CCCCCC") %>%
  row_spec(0, extra_css = "border-bottom: 1px solid black;", background = "#CCCCCC") %>%
  row_spec(nrow(dat.spec), extra_css = "border-bottom: 1px solid black;")



```

## Posterior predictive checks

```{r}
dat.spec <- read.csv("./output/data_Traits.csv", encoding = "latin1")
```


::: {.panel-tabset}

### species


:::: {.panel-tabset}


#### 0.5 km

<div class="scroll4">

```{r, results='asis'}
r <- 500
spec.list <- c("Blaumeise", "Zilpzalp", "Kohlmeise", "Teichrohrsänger", 
                "Rotkehlchen", "Buchfink", "Fitis", "Feldsperling", "Mönchsgrasmücke", 
                "Goldammer", "Rohrammer", "Sumpfrohrsänger", "Heckenbraunelle")

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m1_", sp, "_", r, "m.jpeg"), ")\n")

```
</div>

#### 0.75 km

<div class="scroll4">

```{r, results='asis'}
r <- 750

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m1_", sp, "_", r, "m.jpeg"), ")\n")
 
```
</div>

#### 1 km

<div class="scroll4">

```{r, results='asis'}
r <- 1000

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m1_", sp, "_", r, "m.jpeg"), ")\n")
 
```
</div>

::::


### richness

:::: {.panel-tabset}


#### 0.5 km

<div class="scroll4">

```{r, results='asis'}
r <- 500
spec.list <- c("richness")

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m3_", sp, "_", r, "m.jpeg"), ")\n")

```
</div>

#### 0.75 km

<div class="scroll4">

```{r, results='asis'}
r <- 750

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m3_", sp, "_", r, "m.jpeg"), ")\n")
 
```
</div>

#### 1 km

<div class="scroll4">

```{r, results='asis'}
r <- 1000

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m3_", sp, "_", r, "m.jpeg"), ")\n")
 
```
</div>

::::

### habitat

:::: {.panel-tabset}


#### 0.5 km

<div class="scroll4">

```{r, results='asis'}
r <- 500
trait <- "Habitat"
spec.list <- sort(unique(dat.spec[, trait]))

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m2_", trait, "_", sp, "_", r, "m.jpeg"), ")\n")

```
</div>

#### 0.75 km

<div class="scroll4">

```{r, results='asis'}
r <- 750

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m2_", trait, "_", sp, "_", r, "m.jpeg"), ")\n")
 
```
</div>

#### 1 km

<div class="scroll4">

```{r, results='asis'}
r <- 1000

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m2_", trait, "_", sp, "_", r, "m.jpeg"), ")\n")
 
```
</div>

::::


### habitat density
:::: {.panel-tabset}


#### 0.5 km

<div class="scroll4">

```{r, results='asis'}
r <- 500
trait <- "Habitat.Density"
spec.list <- sort(unique(dat.spec[, trait]))

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m2_", trait, "_", sp, "_", r, "m.jpeg"), ")\n")

```
</div>

#### 0.75 km

<div class="scroll4">

```{r, results='asis'}
r <- 750

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m2_", trait, "_", sp, "_", r, "m.jpeg"), ")\n")
 
```
</div>

#### 1 km

<div class="scroll4">

```{r, results='asis'}
r <- 1000

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m2_", trait, "_", sp, "_", r, "m.jpeg"), ")\n")
 
```
</div>

::::


### migration

:::: {.panel-tabset}


#### 0.5 km

<div class="scroll4">

```{r, results='asis'}
r <- 500
trait <- "Migration"
spec.list <- sort(unique(dat.spec[, trait]))

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m2_", trait, "_", sp, "_", r, "m.jpeg"), ")\n")

```
</div>

#### 0.75 km

<div class="scroll4">

```{r, results='asis'}
r <- 750

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m2_", trait, "_", sp, "_", r, "m.jpeg"), ")\n")
 
```
</div>

#### 1 km

<div class="scroll4">

```{r, results='asis'}
r <- 1000

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m2_", trait, "_", sp, "_", r, "m.jpeg"), ")\n")
 
```
</div>

::::

### lifestyle

:::: {.panel-tabset}


#### 0.5 km

<div class="scroll4">

```{r, results='asis'}
r <- 500
trait <- "Primary.Lifestyle"
spec.list <- sort(unique(dat.spec[, trait]))

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m2_", trait, "_", sp, "_", r, "m.jpeg"), ")\n")

```
</div>

#### 0.75 km

<div class="scroll4">

```{r, results='asis'}
r <- 750

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m2_", trait, "_", sp, "_", r, "m.jpeg"), ")\n")
 
```
</div>

#### 1 km

<div class="scroll4">

```{r, results='asis'}
r <- 1000

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m2_", trait, "_", sp, "_", r, "m.jpeg"), ")\n")
 
```
</div>

::::

### trophic niche
:::: {.panel-tabset}


#### 0.5 km

<div class="scroll4">

```{r, results='asis'}
r <- 500
trait <- "Trophic.Niche"
spec.list <- sort(unique(dat.spec[, trait]))

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m2_", trait, "_", sp, "_", r, "m.jpeg"), ")\n")

```
</div>

#### 0.75 km

<div class="scroll4">

```{r, results='asis'}
r <- 750

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m2_", trait, "_", sp, "_", r, "m.jpeg"), ")\n")
 
```
</div>

#### 1 km

<div class="scroll4">

```{r, results='asis'}
r <- 1000

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m2_", trait, "_", sp, "_", r, "m.jpeg"), ")\n")
 
```
</div>

::::


### Red List GER
:::: {.panel-tabset}


#### 0.5 km

<div class="scroll4">

```{r, results='asis'}
r <- 500
trait <- "RedList_DE"
spec.list <- sort(unique(dat.spec[, trait]))

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m2_", trait, "_", sp, "_", r, "m.jpeg"), ")\n")

```
</div>

#### 0.75 km

<div class="scroll4">

```{r, results='asis'}
r <- 750

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m2_", trait, "_", sp, "_", r, "m.jpeg"), ")\n")
 
```
</div>

#### 1 km

<div class="scroll4">

```{r, results='asis'}
r <- 1000

for(sp in spec.list) cat("![](", paste0("./graphs/prefig/PPC_m2_", trait, "_", sp, "_", r, "m.jpeg"), ")\n")
 
```
</div>

::::


:::

These figures display posterior predictive checks of the response variable bird abundance (for species and trait groups) or species richness (for richness). The left plot shows the distribution of the raw data (dark blue) as well as eight simulations of the dataset based on the respective model (light blue). The right panel shows the ability of the model to reproduce several distributional characteristics of the response variable (dark blue = raw data, light blue = simulated raw data), namely mean, standard deviation (sd), proportion of zero counts (propZ) and maximum values (max). _Reference for habitat density levels: 1 = dense, 2 = semi-open, 3 = open. Reference for Red List categories: * = least concern, 2 = endangered, 3 = vulnerable, 4 = near threatened._


# Results



## Coefficient estimates

```{r}
dat.spec <- read.csv("./output/data_Traits.csv", encoding = "latin1")
dat.spec <- dat.spec[order(dat.spec$species_sci), ]

df.coef <- read.csv("./graphs/prefig/Coefficients.csv")

df.coef <- left_join(df.coef, dat.spec[, c("species", "species_eng")], by = "species")

df.coef$species[!is.na(df.coef$species_eng)] <- df.coef$species_eng[!is.na(df.coef$species_eng)]

coefs <- data.frame("coefficient" = 
                      c("intercept", "weed", "distance", "maize", "AST",   "decade (1st poly)", "decade (2nd poly)"),
                    "coef" = 
                      c("y2016",     "weed", "dist",     "maize", "woodS", "jday1",             "jday2"))

df.coef <- left_join(df.coef, coefs, by = "coef")

df.coef <- df.coef[, c("model", "radius", "species", "coefficient", "median", "lwr", "upr", "BayesP")]
colnames(df.coef) <- c("model", "radius", "species", "coefficient", "median", "lwr", "upr", "P(β>0)")

df.coef <- df.coef %>% arrange(species)

df.coef[, 5:8] <- round(df.coef[, 5:8], 2)

table1 <- function(model, rad){
  
  tab1 <- df.coef[df.coef$model == model & df.coef$radius == rad, 3:8]
  nspec <- length(unique(tab1$species))
  n <- 0
  lines <- NULL
  for(i in 1:nspec) {
    n <- n + 7
    lines <- c(lines, n)
  }

  tab.cap <- "Coefficient estimates (median and 95% CrI (lwr, upr)) of main predictors per trait-group. P(β>0) is the Bayesian posterior probability of the coefficient to exceed zero. For slopes, values close to 0 indicate a decreasing relationship between abundance and the respective predictor, values close to 1 an increasing relationship."
  
  if(model == "species") {
    tab.cap <- "Coefficient estimates (median and 95% CrI (lwr, upr)) of main predictors per species. P(β>0) is the Bayesian posterior probability of the coefficient to exceed zero. For slopes, values close to 0 indicate a decreasing relationship between abundance and the respective predictor, values close to 1 an increasing relationship."
    }
  
  if(model == "Diversity") {
    tab1 <- select(tab1, -species)
    tab.cap <- "Coefficient estimates (median and 95% CrI (lwr, upr)) of main predictors. P(β>0) is the Bayesian posterior probability of the coefficient to exceed zero. For slopes, values close to 0 indicate a decreasing relationship between species richness and the respective predictor, values close to 1 an increasing relationship."
  }
  
  if(model == "Habitat.Density") {
    tab.cap <- "Coefficient estimates (median and 95% CrI (lwr, upr)) of main predictors per trait-group. P(β>0) is the Bayesian posterior probability of the coefficient to exceed zero. For slopes, values close to 0 indicate a decreasing relationship between abundance and the respective predictor, values close to 1 an increasing relationship. <em>Habitat density levels: 1 = dense, 2 = semi-open, 3 = open.</em>"
  }
  
  if(model == "RedList_DE") {
    tab.cap <- "Coefficient estimates (median and 95% CrI (lwr, upr)) of main predictors per trait-group. P(β>0) is the Bayesian posterior probability of the coefficient to exceed zero. For slopes, values close to 0 indicate a decreasing relationship between abundance and the respective predictor, values close to 1 an increasing relationship. <em>Red List categories: * = least concern, 2 = endangered, 3 = vulnerable, 4 = near threatened.</em>"
  }
    
  tab <- kbl(tab1, caption = tab.cap,     
      row.names = F, escape = FALSE) %>%
    kable_styling(full_width = FALSE, position = "left") %>%
    scroll_box(height = "750px") %>%
  
    row_spec(0:nrow(tab1), extra_css = "font-size:14px;") %>%
  
    row_spec(0, extra_css = "border-bottom: 1px solid black;", background = "#CCCCCC") %>%
    row_spec(lines, extra_css = "border-bottom: 1px solid black;")
  
  return(tab)
  
}

```

::: {.panel-tabset}

### species

:::: {.panel-tabset}

#### 0.5 km
```{r}
model <- "species"
table1(model = model, rad = 0.5)
```

#### 0.75 km
```{r}
table1(model = model, rad = 0.75)
```

#### 1 km
```{r}
table1(model = model, rad = 1)
```

::::

### richness

:::: {.panel-tabset}

#### 0.5 km
```{r}
model <- "Diversity"
table1(model = model, rad = 0.5)
```

#### 0.75 km
```{r}
table1(model = model, rad = 0.75)
```

#### 1 km
```{r}
table1(model = model, rad = 1)
```

::::

### habitat

:::: {.panel-tabset}

#### 0.5 km
```{r}
model <- "Habitat"
table1(model = model, rad = 0.5)
```

#### 0.75 km
```{r}
table1(model = model, rad = 0.75)
```

#### 1 km
```{r}
table1(model = model, rad = 1)
```

::::

### habitat density

:::: {.panel-tabset}

#### 0.5 km
```{r}
model <- "Habitat.Density"
table1(model = model, rad = 0.5)
```

#### 0.75 km
```{r}
table1(model = model, rad = 0.75)
```

#### 1 km
```{r}
table1(model = model, rad = 1)
```

::::

### migration

:::: {.panel-tabset}

#### 0.5 km
```{r}
model <- "Migration"
table1(model = model, rad = 0.5)
```

#### 0.75 km
```{r}
table1(model = model, rad = 0.75)
```

#### 1 km
```{r}
table1(model = model, rad = 1)
```

::::

### lifestyle

:::: {.panel-tabset}

#### 0.5 km
```{r}
model <- "Primary.Lifestyle"
table1(model = model, rad = 0.5)
```

#### 0.75 km
```{r}
table1(model = model, rad = 0.75)
```

#### 1 km
```{r}
table1(model = model, rad = 1)
```

::::

### trophic niche

:::: {.panel-tabset}

#### 0.5 km
```{r}
model <- "Trophic.Niche"
table1(model = model, rad = 0.5)
```

#### 0.75 km
```{r}
table1(model = model, rad = 0.75)
```

#### 1 km
```{r}
table1(model = model, rad = 1)
```

::::

### Red List GER

:::: {.panel-tabset}

#### 0.5 km
```{r}
model <- "RedList_DE"
table1(model = model, rad = 0.5)
```

#### 0.75 km
```{r}
table1(model = model, rad = 0.75)
```

#### 1 km
```{r}
table1(model = model, rad = 1)
```

::::

:::


```{r}
dat.spec  <- read.csv("./data/spec_info.csv", fileEncoding = "latin1")[, c("species", "species_eng")]

f.list <- list.files("./output", pattern = "500m.csv")
f.list <- f.list[grep("Predictions_m", f.list)]

pp <- list()

for(i in 1:length(f.list)) pp[[i]] <- read.csv(paste0("./output/", f.list[i]), fileEncoding = "latin1")
pp[[1]] <- left_join(pp[[1]], dat.spec, by = "species")
pp[[4]]$species <- as.character(pp[[4]]$species)

# plot predictions per predictor
plot3 <- function(preds = pp, x = NULL, y = "fit", 
                  xlab = NULL, ylab = "density [ind. per 1000 hm²]", 
                  basesize = 12, color = NULL, c.label = NULL, NSpec = NULL){
  preds$x     <- preds[,x]
  preds$y     <- preds[,y]
  preds$color <- as.factor(preds[,color])
  if(is.null(NSpec)) NSpec <- FALSE

 
  # base plot 
  g <- ggplot() +
    theme_light(base_size = basesize)
  
  # add predictions
  if(!NSpec){
    g <- g +
      
      geom_line(data = preds, aes(x = x, y = y, color = color), lwd = 1.2) +
      
      scale_color_viridis_d(c.label, direction = -1, end = 0.9) +
    xlab(xlab) +
    ylab(ylab) +
    ylim(0, NA)
  }
  
  if(NSpec){
    g <- g +
      
      geom_line(data = preds, aes(x = x, y = y), color = "grey", lwd = 1.2) +

    xlab(xlab) +
    ylab("number of species") +
    ylim(0, NA)
  }
  
  if(x == "julian_day"){
    x.val <- c(21.3, 24.4, 27.4, 30.5)
    x.lab <- c("1st Aug", "1st Sep", "1st Oct", "1st Nov")

    g <- g + scale_x_continuous(sec.axis = dup_axis(
      name = "date",
      labels = x.lab,
      breaks = x.val
    ))
  }
  

  return(g)
  
}


plot3_plotly <- function(preds = pp, x = NULL, y = "fit", 
                         xlab = NULL, ylab = "density [ind. per 1000 hm²]", 
                         basesize = 12, color = NULL, c.label = NULL, NSpec = NULL){

  library(plotly)

  preds$x     <- preds[[x]]
  preds$y     <- preds[[y]]

  if (!is.null(color)) {
    preds$color <- as.factor(preds[[color]])
  }

  if (is.null(NSpec)) NSpec <- FALSE

  p <- plot_ly()

  # ---- add lines ----
  if (!NSpec) {
    p <- p %>%
      add_lines(
        data = preds,
        x = ~x,
        y = ~y,
        color = ~color,
        colors = viridisLite::viridis(length(unique(preds$color)), direction = -1, end = 0.9),
        line = list(width = 4)
      )
  }

  if (NSpec) {
    p <- p %>%
      add_lines(
        data = preds,
        x = ~x,
        y = ~y,
        line = list(color = "grey", width = 4),
        showlegend = FALSE
      )
    ylab <- "number of species"
  }

  # ---- base layout ----
  p <- p %>%
    layout(
      template = "none",  # remove default plotly theme
      
      plot_bgcolor = "white",
      paper_bgcolor = "white",
      
      xaxis = list(
        title = xlab,
        showgrid = TRUE,
        gridcolor = "grey80",
        zeroline = FALSE,
        showline = TRUE,
        linecolor = "grey60",
        mirror = TRUE
      ),
      
      yaxis = list(
        title = ylab,
        showgrid = TRUE,
        gridcolor = "grey80",
        zeroline = FALSE,
        showline = TRUE,
        linecolor = "grey60",
        mirror = TRUE,
        rangemode = "tozero"
      )
    )
  # ---- secondary x-axis (THIS is the key part) ----
  if (x == "julian_day") {

    x.val <- c(21.3, 24.4, 27.4, 30.5, 21.3, 24.4, 27.4, 30.5)
    x.lab <- c("Aug, 1st", "Sep, 1st", "Oct, 1st", "Nov, 1st", "|", "|", "|", "|")
    y.val <- c(1.03, 1.03, 1.03, 1.03, 0.99, 0.99, 0.99, 0.99)

    ann <- lapply(seq_along(x.val), function(i) {
      list(
        x = x.val[i],
        y = y.val[i],              # slightly above plot
        xref = "x",
        yref = "paper",
        text = x.lab[i],
        showarrow = FALSE,
        xanchor = "center",
        yanchor = "bottom"
      )
    })
    
    p <- p %>%
      layout(
        annotations = ann,
        margin = list(t = 60)  # give space for top labels
      )
  }

  return(p)
}



```

## Posterior predictions

::: {.panel-tabset}

### species

:::: {.panel-tabset}

#### maize

```{r}
tmp <- pp[[1]]
plot3_plotly(x = "maize_p", xlab = maize500, preds = tmp[tmp$pred == "maize",], 
           color = "species_eng", c.label = "species")
```

#### AST

```{r}
plot3_plotly(x = "woodS_p", xlab = woodS500, preds = tmp[tmp$pred == "woodS",], 
           color = "species_eng", c.label = "species")
```

#### distance
```{r}
plot3_plotly(x = "distance_m", xlab = dist, preds = tmp[tmp$pred == "dist",], 
           color = "species_eng", c.label = "species")
```

#### weed
```{r}
plot3_plotly(x = "weed_p", xlab = weed, preds = tmp[tmp$pred == "weed",], 
           color = "species_eng", c.label = "species")
```

#### decade
```{r}
plot3_plotly(x = "julian_day", xlab = jday, preds = tmp[tmp$pred == "jday",], 
           color = "species_eng", c.label = "species")

```
::::

### richness

:::: {.panel-tabset}

#### maize

```{r}
tmp <- pp[[8]]
plot3_plotly(x = "maize_p", xlab = maize500, preds = tmp[tmp$pred == "maize",], 
           color = "species", c.label = "species", NSpec = TRUE)

```

#### AST

```{r}
plot3_plotly(x = "woodS_p", xlab = woodS500, preds = tmp[tmp$pred == "woodS",], 
           color = "species", c.label = "species", NSpec = TRUE)

```

#### distance
```{r}
plot3_plotly(x = "distance_m", xlab = dist, preds = tmp[tmp$pred == "dist",], 
           color = "species", c.label = "species", NSpec = TRUE)

```

#### weed
```{r}
plot3_plotly(x = "weed_p", xlab = weed, preds = tmp[tmp$pred == "weed",], 
           color = "species", c.label = "species", NSpec = TRUE)

```

#### decade
```{r}
plot3_plotly(x = "julian_day", xlab = jday, preds = tmp[tmp$pred == "jday",], 
           color = "species", c.label = "species", NSpec = TRUE)

```
::::


### habitat

:::: {.panel-tabset}

#### maize

```{r}
tmp <- pp[[2]]
plot3_plotly(x = "maize_p", xlab = maize500, preds = tmp[tmp$pred == "maize",], 
           color = "species", c.label = "habitat")

```

#### AST

```{r}
plot3_plotly(x = "woodS_p", xlab = woodS500, preds = tmp[tmp$pred == "woodS",], 
           color = "species", c.label = "habitat")

```

#### distance
```{r}
plot3_plotly(x = "distance_m", xlab = dist, preds = tmp[tmp$pred == "dist",], 
           color = "species", c.label = "habitat")

```

#### weed
```{r}
plot3_plotly(x = "weed_p", xlab = weed, preds = tmp[tmp$pred == "weed",], 
           color = "species", c.label = "habitat")

```

#### decade
```{r}
plot3_plotly(x = "julian_day", xlab = jday, preds = tmp[tmp$pred == "jday",], 
           color = "species", c.label = "habitat")

```
::::

### habitat density

:::: {.panel-tabset}

#### maize

```{r}
tmp <- pp[[3]]
plot3_plotly(x = "maize_p", xlab = maize500, preds = tmp[tmp$pred == "maize",], 
           color = "species", c.label = "habitat density")

```

#### AST

```{r}
plot3_plotly(x = "woodS_p", xlab = woodS500, preds = tmp[tmp$pred == "woodS",], 
           color = "species", c.label = "habitat density")

```

#### distance
```{r}
plot3_plotly(x = "distance_m", xlab = dist, preds = tmp[tmp$pred == "dist",], 
           color = "species", c.label = "habitat density")

```

#### weed
```{r}
plot3_plotly(x = "weed_p", xlab = weed, preds = tmp[tmp$pred == "weed",], 
           color = "species", c.label = "habitat density")

```

#### decade
```{r}
plot3_plotly(x = "julian_day", xlab = jday, preds = tmp[tmp$pred == "jday",], 
           color = "species", c.label = "habitat density")

```
::::

### migration

:::: {.panel-tabset}

#### maize

```{r}
tmp <- pp[[4]]
plot3_plotly(x = "maize_p", xlab = maize500, preds = tmp[tmp$pred == "maize",], 
           color = "species", c.label = "migration")

```

#### AST

```{r}
plot3_plotly(x = "woodS_p", xlab = woodS500, preds = tmp[tmp$pred == "woodS",], 
           color = "species", c.label = "migration")

```

#### distance
```{r}
plot3_plotly(x = "distance_m", xlab = dist, preds = tmp[tmp$pred == "dist",], 
           color = "species", c.label = "migration")

```

#### weed
```{r}
plot3_plotly(x = "weed_p", xlab = weed, preds = tmp[tmp$pred == "weed",], 
           color = "species", c.label = "migration")

```

#### decade
```{r}
plot3_plotly(x = "julian_day", xlab = jday, preds = tmp[tmp$pred == "jday",], 
           color = "species", c.label = "migration")

```
::::

### lifestyle

:::: {.panel-tabset}

#### maize

```{r}
tmp <- pp[[5]]
plot3_plotly(x = "maize_p", xlab = maize500, preds = tmp[tmp$pred == "maize",], 
           color = "species", c.label = "lifestyle")

```

#### AST

```{r}
plot3_plotly(x = "woodS_p", xlab = woodS500, preds = tmp[tmp$pred == "woodS",], 
           color = "species", c.label = "lifestyle")

```

#### distance
```{r}
plot3_plotly(x = "distance_m", xlab = dist, preds = tmp[tmp$pred == "dist",], 
           color = "species", c.label = "lifestyle")

```

#### weed
```{r}
plot3_plotly(x = "weed_p", xlab = weed, preds = tmp[tmp$pred == "weed",], 
           color = "species", c.label = "lifestyle")

```

#### decade
```{r}
plot3_plotly(x = "julian_day", xlab = jday, preds = tmp[tmp$pred == "jday",], 
           color = "species", c.label = "lifestyle")

```
::::

### trophic niche

:::: {.panel-tabset}

#### maize

```{r}
tmp <- pp[[7]]
plot3_plotly(x = "maize_p", xlab = maize500, preds = tmp[tmp$pred == "maize",], 
           color = "species", c.label = "trophic niche")

```

#### AST

```{r}
plot3_plotly(x = "woodS_p", xlab = woodS500, preds = tmp[tmp$pred == "woodS",], 
           color = "species", c.label = "trophic niche")

```

#### distance
```{r}
plot3_plotly(x = "distance_m", xlab = dist, preds = tmp[tmp$pred == "dist",], 
           color = "species", c.label = "trophic niche")

```

#### weed
```{r}
plot3_plotly(x = "weed_p", xlab = weed, preds = tmp[tmp$pred == "weed",], 
           color = "species", c.label = "trophic niche")

```

#### decade
```{r}
plot3_plotly(x = "julian_day", xlab = jday, preds = tmp[tmp$pred == "jday",], 
           color = "species", c.label = "trophic niche")

```
::::


### Red List GER

:::: {.panel-tabset}

#### maize

```{r}
tmp <- pp[[6]]
plot3_plotly(x = "maize_p", xlab = maize500, preds = tmp[tmp$pred == "maize",], 
           color = "species", c.label = "Red List GER")

```

#### AST

```{r}
plot3_plotly(x = "woodS_p", xlab = woodS500, preds = tmp[tmp$pred == "woodS",], 
           color = "species", c.label = "Red List GER")

```

#### distance
```{r}
plot3_plotly(x = "distance_m", xlab = dist, preds = tmp[tmp$pred == "dist",], 
           color = "species", c.label = "Red List GER")

```

#### weed
```{r}
plot3_plotly(x = "weed_p", xlab = weed, preds = tmp[tmp$pred == "weed",], 
           color = "species", c.label = "Red List GER")

```

#### decade
```{r}
plot3_plotly(x = "julian_day", xlab = jday, preds = tmp[tmp$pred == "jday",], 
           color = "species", c.label = "Red List GER")

```
::::


:::

These figures display the predicted bird densities (for species and trait groups) or species richness (for richness) for different covariates (proportion of maize, proportion of AST, distance to field margin, weed infestation, decade of the year). The used reference radius is 0.5 km. When predicting for one covariate, other covariates were set to their mean values. The figures are interactive, so you can remove groups by selecting them in the legend or hover across lines to show exact values. _Reference for habitat density levels: 1 = dense, 2 = semi-open, 3 = open. Reference for Red List categories: * = least concern, 2 = endangered, 3 = vulnerable, 4 = near threatened._

